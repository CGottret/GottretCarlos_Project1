...
int rw_read_assignment_info(FILE *fptr, int original_scores[], int weights[], int penalties[], double weighted_scores[], int penalized_scores[], int size) {
    char line[52];
    int total_weights = 0, i, j;
    for (i = 0; i < size; i++) {
        char *line_arr[4];
        char delim[] = ", ";
        char *ptr;
